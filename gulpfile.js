...
gulp.task('include', function() {
    gulp.src(['cwd/templates/**/*.html']).pipe(plumber({
      errorHandler: onError
    })).pipe(includehtml({basePath: 'cwd/includes'})).pipe(gulp.dest('./render/templates/')).pipe(reload({
        stream: true
    }));
...
gulp.task('image-move', function() {
    return gulp.src('cwd/assets/images/*').pipe(gulp.dest('render/assets/images/')).pipe(size({
        showFiles: true
    }));
});
/*============================================
...
=            Task Runners             =
=====================================*/
gulp.task('default', ['include', 'sass', 'js', 'image-move', 'fonts']);
